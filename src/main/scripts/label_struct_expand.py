...
comment_start = "/*"
comment_end = "*/"
comment_start_replacement = "/#*"
comment_end_replacement = "*#/"
...
class LabelUtil:
    ...
    def get_label_value(cls, line):
        ...
        if '=' in line:
            ...
            if comment_start in value:
                cmt_idx = value.index(comment_start)
                value = value[0:cmt_idx]
                value = value.strip()
            # check for any whitespace for sanity
            if " " in value:
                line_parts = line.split(" ")
                value = line_parts[0]
                value = value.strip()
...
class StructExpander:
    ...
    def perform_replacement(self, contents, line, fmt_path):
        ...
        if insert_list is None:
            ...
            warning_cmt = f"{comment_start} Error: Was unable to read contents from {fmt_path} {comment_end}"
        ...
        # Original line might have a label comment, which will cause headaches
        # when we create a new comment using that line.  So go through and replace
        # those commend end-bits (still want to keep text 'cuz it could be useful)
        line_prefix =  "Original line"
        line_suffix = line
        if comment_start in line or comment_end in line:
            line_prefix =  "Original line w/ reformatted comment"
            line_suffix = line_suffix.replace(comment_start, comment_start_replacement)
            line_suffix = line_suffix.replace(comment_end, comment_end_replacement)
            
        # Comments to be included in the inserted list
        comment_1 = f"{comment_start} Replacing ^STRUCTURE with referenced file contents {comment_end}"
        comment_2 = f"{comment_start} {line_prefix}: {line_suffix} {comment_end}"
        comment_3 = f"{comment_start} START_EXPANSION: {fmt_path} {comment_end}"
        comment_N = f"{comment_start} END_EXPANSION: {fmt_path} {comment_end}"
