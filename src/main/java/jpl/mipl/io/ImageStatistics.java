	...
	**/
	...
	public void loopStats(RenderedImage ri) {
	  ...
	  public void loopStats(RenderedImage ri, int bandIndex) {
		  ...
		  //Checks added per 2020 security scan
	      if ( meanSampleCt == null || medianSampleCt == null || mean == null ||
	           median == null       || min == null            ||  max == null ||  
	           sum == null          || std_dev == null )
	      {
	          System.out.println("ImageStatitics::loopStats: statistics fields have not been initialized");
	          return;
	      }

	      if ( bandIndex >= meanSampleCt.length || bandIndex >= medianSampleCt.length || 
	           bandIndex >= mean.length         || bandIndex >= median.length         || 
	           bandIndex >= min.length          || bandIndex >= max.length            ||  
	           bandIndex >= sum.length          || bandIndex >= std_dev.length )
	      {
	          System.out.println("ImageStatitics::loopStats: bandIndex '"+bandIndex+"' exceeds statistics fields length");
              return;
	      }    
