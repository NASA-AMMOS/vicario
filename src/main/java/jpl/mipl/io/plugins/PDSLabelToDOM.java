...
public final class PDSLabelToDOM {
  ...
  // new srl 5-18 parse SFDU labels also. Different first line. The rest of the label is the same
  Pattern sfduLabelP  = Pattern.compile("^CCSD\\w*$");
  Pattern sfduLabelExceptionP  = Pattern.compile("^CCSD\\w*\\s*=\\s*SFDU_LABEL$");
  ...
  **/
  ...
  private HashMap processLine(String line, int lineCt, HashMap nodeLevelHM) {
      ...
      Matcher  sfduLabelM, sfduLabelExceptionM ;
      ...
      if (debug && lineCt > 345 && lineCt < 360) {
      ...
      ***/
      ...
      sfduLabelM = sfduLabelP.matcher(line);
      sfduLabelExceptionM = sfduLabelExceptionP.matcher(line);
      ...
      // else if (line.startsWith( "GROUP ")) {
    	  ...
    	  // it is possible to have an item which is GROUP_SOMETHING
          // open a new object, put new items inside this one
          if (debug) {
              for (int i=0 ; i< level ; i++) { _output.print(" "); }
          ...
          currentNode.appendChild(element);
          if (debug) {
              System.out.println(lineCt+")"+level+">appendChild  key="+key+" value="+value+"<  GROUP");
          }          
          currentNode = (Node) element;
      }      
      ...
      } else if (    sfduLabelM.find() ) {
    	  if (debug) {
              System.out.println(lineCt+") ProcessLine sfduLabelM>"+line);
          }
    	  // add nodes for all this stuff
    	  key = "SFDU";
          element = (Element) _document.createElement(key);
      ...
      } else if ( sfduLabelExceptionM.find() ) {   
    	  sv = line.split("=");
          if (debug) {
              System.out.println(lineCt+") ProcessLine sfduLabelExceptionM>"+line);
          }
          // add nodes for all this stuff
          key = "SFDU";
          element = (Element) _document.createElement(key);
          ...
          if (value.startsWith("\"")) {
              ...
              if (debug) {
                  System.out.println(lineCt+")"+level+">appendChild  key="+key+" value="+value+"<  \\\"");
                      ...
                      for (int i=0 ; i< level ; i++) { _output.print(" "); }
                      _output.println("<item>"+key+" "+value+"</item>" );
                      _output.flush();
              ...
              if (useSubitem == false) {
                  ...
                  // check if this value is in quotes
                  element = (Element) _document.createElement("item");
                  element.setAttribute(keyTag,key);
                  element.setAttribute("quoted",quoted);
                  ...
                  for (int i =0; i<subitems.length ; i++) {
                      ...
                      subv = subv.trim();
                      // value = value.replaceAll("\"","");
                      // determine if this subitem is quoted
                      if (subv.indexOf("\"") == -1) {
                          quoted = "false";
                      } else {
                    	  quoted = "true";
                      ...
                      if (debug) {
                    	  ...
                    	  for (int j=0 ; j< level ; j++) { _output.print(" "); }
                          _output.println("<subitem>"+key+" "+subv+"</subitem>" );
                          _output.flush();
                      }
  ...
  String[] value_split(String v) {
	  ...
	  for (int i=0 ; i<cbLen; i++) {
		  ...
		  if (c == '\"') {
			 ...
			 // keep the quotes, later we will remove them. we need to know which elements are quoted
  ...
  private String joinLines(String line1, String line2) {
      ...
      // for now just do it
	  // find a comment that has no space between /*text
      // add a space between /* */ \S is non whitespace
	  // Pattern commentLineStartP = Pattern.compile("^/\\*\\p{Print}+");
	  Pattern commentLineStartP = Pattern.compile("^/\\*\\S");
	  Matcher commentLineStartM = commentLineStartP.matcher(aLine);

	  if (commentLineStartM.find()) {
		  if (debug) {
	          System.out.println("joinLines comment line start found before"+aLine);  
		  }
		// add in a space after /*
		  aLine = aLine.replaceAll("\\/\\*", "\\/\\* ");
		  if (debug) {
	          System.out.println("joinLines comment line start found after"+aLine);  
		  }
	  }
	  // do one for commas and periods
	  Pattern commentLineEndP = Pattern.compile("^/\\*\\S");
	  Matcher commentLineEndM = commentLineStartP.matcher(aLine);

	  if (commentLineEndM.find()) {
		  if (debug) {
	          System.out.println("joinLines comment line end found before"+aLine);  
		  }
		// add in a space after /*
		  aLine = aLine.replaceAll("\\*\\/", " \\*\\/");
		  if (debug) {
  ...
  **/
  public Document  buildDocument() {
    ...
    // use to capture a multi-line comment
    ...
    **/
     ...
     **/
     ...
     // create a pattern that is a keyword = value /* comment */. Lines are trimmed of leading and trailing spaces
     //                                                             key     space = space value space /* comment */
     Pattern keywordSpaceEqualsValueCommentLineP = Pattern.compile("[A-Z_0-9]+\\s*=\\s*\\p{Print}+\\s*/\\*\\p{Print}+\\*/");
  //else if (keywordSpaceEqualsValueCommentLineM.find()) {
     // create a pattern that is a keyword = value /* comment start,finished on another line
     ...
     Matcher keywordSpaceEqualsLineM, keywordSpaceEqualsValueLineM;
     Matcher keywordSpaceEqualsValueCommentLineM;
     Matcher commentLineM, commentLineStartM ;
     Matcher keywordEqualsLineM, keywordEqualsValueLineM;
      ...
      while((line = readLine()) != null) {
          ...
          if (debug) {
              ...
              System.out.println(lineCt+") inValue = "+inValue+" inComment = "+inComment+" trLine ="+trLine+"<");
          }
          
          nonEmptyLineM = nonEmptyLineP.matcher(line);
          commentLineM = commentLineP.matcher(trLine);
          commentLineStartM = commentLineStartP.matcher(trLine);
          ...
          keywordSpaceEqualsValueCommentLineM = keywordSpaceEqualsValueCommentLineP.matcher(trLine);
          ...
          if (nonEmptyLineM.find()) {
               ...
               if (trLine.equals("END") ) {
                	  ...
                	  if (debug) { 
                		  System.out.println(lineCt+")  #### inComment ### collectedLine >"+collectedLine+"< trLine >"+trLine+"<");                		  
                	  }
                      // look for the end of the comment
                	  if (trLine.endsWith("*/")){
                		  if (debug) { System.out.println(lineCt+")  #### inComment ### endsWith */"); }              	
                		  collectedLine = joinLines(collectedLine, trLine);
                		  nodeLevelHM = processLine(collectedLine, lineCt, nodeLevelHM);
                		  inComment = false;
                		  collectedLine = "";
                	  } else {
                		  if (debug) { System.out.println(lineCt+")  #### inComment ### joinLines");     }
                		  collectedLine = joinLines(collectedLine, trLine);
                	  }
                	  ...
                	  if (debug) { 
                		  System.out.println(lineCt+")  #### equals ### END_GROUP or END_OBJECT trLine >"+trLine+"<");                		  
                	  }
          			...
          			if (debug) { 
          				System.out.println(lineCt+")  #### startsWith ### END_GROUP or END_OBJECT trLine >"+trLine+"<"); 
                	  ...
                	  // must check this before any of the comment checks are done
                	  int qCt = countStrInStr("\"", trLine);
                	  int qCt2 = countStrInStr("\"", collectedLine);
                	  if (debug) { 
                		  System.out.println(lineCt+")  ####### keywordSpaceEqualsValueComment qCt="+qCt+" qCt2="+qCt2+"  inValue = "+inValue+"  trLine >"+trLine+"<"); 
                		  System.out.println(lineCt+")  ####### keywordSpaceEqualsValueComment collectedLine ="+collectedLine+"<"); 
                		  }
               	      
                	  // pull the comment out of the line or will processLine handle it?
                	  // this line ends with the comment
                	  int comment_start_index = trLine.indexOf("/*");
                	  System.out.printf(lineCt+")  ####### keywordSpaceEqualsValueComment comment_start_index=%d \n", comment_start_index);
                	  String kv = "";
            		  String comment = "";
                	  if (comment_start_index != -1) {
                		  kv = trLine.substring(0, comment_start_index-1);
                		  comment = trLine.substring(comment_start_index);
                		  System.out.printf(lineCt+")  ####### kv=%s  comment=%s\n", kv, comment);
                		  trLine = kv.trim();
                	  }
                	  ...
                	  if (inValue) {
                		  ...
                		  collectedLine = trLine;
                		  if (comment.length() > 0) {
                			  System.out.printf(lineCt+")  ####### processLine  comment=%s\n", comment);
                			  nodeLevelHM = processLine(comment, lineCt, nodeLevelHM);
                		  }
                  ...
                  else if (commentLineStartM.find()) {
                	  if (debug) { System.out.println(lineCt+")  ####### commentLineStart trLine >"+trLine+"<"); }
                      // the previous line is complete
                      nodeLevelHM = processLine(collectedLine, lineCt, nodeLevelHM);
                      // start a new line with this comment fragment
                      collectedLine = trLine;
                  ...
                  else if (trLine.endsWith("*/")) {
                	  if (debug) { System.out.println(lineCt+")  ####### commentLine ends trLine >"+trLine+"<"); }
                	  collectedLine = joinLines(collectedLine, trLine);
                	  nodeLevelHM = processLine(collectedLine, lineCt, nodeLevelHM);
                      // start a new line
                      // collectedLine = trLine;
                	  ...
                	  if (debug) { 
                		  System.out.println(lineCt+")  ####### No Match trLine >"+trLine+"<"); 
                		  System.out.println(lineCt+")  ####### collectedLine >"+collectedLine+"<"); 
                		  }
                   // collectedLine += s;
                      collectedLine = joinLines(collectedLine, trLine);
                      if (debug) { 
                		  System.out.println(lineCt+")  ####### after joinLines collectedLine >"+collectedLine+"<"); 
                      }
                	...
                	// we need to ignore any parens that are inside of quotes
                	  // will also do this for brace and bracket
                      if (qCt == 1 && startParenCt > 0 && endParenCt >0 ) {
                    	  if (debug) { 
                        	  System.out.println(lineCt+")  startParenCt = "+startParenCt+"   endParenCt = "+endParenCt+"  qCt = "+qCt+" @#@#"); 
                    	  }
                    	  if (firstQuoteIndex < firstParenIndex) {
                    		  firstParenIndex = 0;
                    		  lastParenIndex = 0;
                    		  startParenCt = 0;
                    		  endParenCt = 0;
                    		  if (debug) { 
                            	  System.out.println(lineCt+")  zeroing out parenCt and partenIndex "); 
                        	  }
                    	  }
                      ...
                      if (debug) { 
                    	  System.out.println(lineCt+")  ******************************************************************"); 
                    	  System.out.println(lineCt+")  inValue = "+inValue+"   inComment = "+inComment+"  "); 
                    	  System.out.println(lineCt+")  startParenCt = "+startParenCt+"   endParenCt = "+endParenCt+"  qCt = "+qCt); 
                      ...
                      } else if (startBraceCt == 0 && startBracketCt == 0 && startParenCt == 0 && qCt == 2) {
                    	  // closed the quote and no parens
                    	  if (debug) { System.out.println(lineCt+")  startBraceCt == 0 && startBracketCt == 0 && startParenCt == 0 && qCt == 2"); }
                    	  nodeLevelHM = processLine(collectedLine, lineCt, nodeLevelHM);
                    	  // start a new line
                    	  collectedLine = "";
                    	  inValue = false;
                  ...
                  if (debug) {
                      ...
                      System.out.println(" "+lineCt+")  collectedLine ="+collectedLine+"<");
                      System.out.println(" "+lineCt+")  inValue = "+inValue+" inComment = "+inComment+" keepReadingInner = "+keepReadingInner+" keepReadingOuter = "+keepReadingOuter);
                  }
            }
