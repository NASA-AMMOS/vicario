...
public class PDSImageWriter extends ImageWriter {
    ...
    **/
    ...
    private void write(RenderedImage ri, PDSMetadata pm) {     
        ...
        boolean debugPerf = debug;
        //boolean debugPerf = true;
        if (debugPerf) System.out.println((new Date()) + " beginning label write");
...
//          remove these items from the pdsDoc 
    		...
    		*/
            ...
            if (detachedLabelOnly && fakeImage)  {
                ...
                ***/
                ...
                if (debug) { 
                	System.out.println("PDSImageWriter.write() imageStartNode "+imageStartNode );
                	System.out.println("PDSImageWriter.write() fileRecordsNode "+fileRecordsNode );
                	domUtil.serializeNode(fileRecordsNode, "fileRecordsNode.xml", "xml"); 
                	}
                
                xPath = "//item[@key='RECORDS_BYTES']";    
                Node recordBytesNode = domUtil.getSingleNode(doc, xPath);
                if (debug) { 
                	System.out.println("PDSImageWriter.write() recordBytesNode "+recordBytesNode );
                	System.out.println("PDSImageWriter.write() fileRecordsNode "+fileRecordsNode );
                	domUtil.serializeNode(recordBytesNode, "recordBytesNode.xml", "xml"); 
                	}
                /***
                ...
                ***/
                ...
                if (fileRecordsNode != null) {
                	if (debug) System.out.println("PDSImageWriter.write() pds_label.insertBefore(fileRecordsNode, imageStartNode); " );
                	pds_label.insertBefore(fileRecordsNode, imageStartNode);
                } else  if (recordBytesNode != null) {
                	if (debug) System.out.println("PDSImageWriter.write() pds_label.insertBefore(recordBytesNode, imageStartNode); " );
                	pds_label.insertBefore(recordBytesNode, imageStartNode);
                } else {
                	// pds_label.
                	if (debug) System.out.println("PDSImageWriter.write() DON'T pds_label.insertBefore(); " );
                	// ???
                }
                
                // check if fileRecordsNode exists. 
            ...
            **/
            ...
            // domUtil.setDebug(debug);
             ...
             jpl.mipl.io.vicar.SystemLabel systemLabel = null;;
           ...
           // then that method will detect the Class and do something good
            ...
            long numReads = 0, numWrites = 0;
            long readMS = 0, writeMS = 0;
            long start = System.currentTimeMillis();
            long lastSpew = start;
            int numTiles = (txmax - txmin + 1) * (tymax - tymin + 1);
            if (debugPerf) {
                System.out.println((new Date()) + " finished label write, beginning tile conversion");
                System.out.println("tile size is " + tileWidth + "x" + tileHeight +
                                   "; tile grid is " + (txmax - txmin + 1) + "x" + (tymax - tymin + 1));
            }
            for (tj = tymin; tj <= tymax; tj++) {
                ...
                if (debugPerf && (now - lastSpew > 10 * 1000 || (tj == tymax && ti == txmax))) {
                    double rmp = 1e-6 * tileWidth * tileHeight * numReads;
                    double wmp = 1e-6 * tileWidth * tileHeight * numWrites;
                    double sec = 1e-3 * (now - start);
                    System.out.printf("read %d/%d tiles, %.2fMpx, %.2fMpx/s; wrote %d/%d tiles, %.2fMpx, %.2fMpx/s\n",
                                      numReads, numTiles, rmp, rmp / sec, numWrites, numTiles, wmp, wmp / sec);
                    lastSpew = now;
                }
            ...
            if (debugPerf) System.out.println((new Date()) + " finished tile conversion");
