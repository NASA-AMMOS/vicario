...
$label.core_base= sys.getCore_base();
    ...
    public String getPds4data_type() {
    	...
    	if (debug) {
    		...
    		System.out.println("ImageToPDS_DOM.getPds4data_type() readerFormat = "+readerFormat+"    ");
    	}
    	if (!pds3SampleType.isEmpty() && sample_bits > 0) {
    		// if these are set override the RenderedImage data type
    		// if these values were not set we don't want to override
    		if (debug) {
    			System.out.println("ImageToPDS_DOM.getPds4data_type() from pds3SampleType & sample_bits");
    			new Throwable().printStackTrace();
    		}
    		 ...
    		 * RIEEE LSB ??, IEEE MSB ??
    		 * LOW - x86-linux vax intel
             * HIGH - sun java PPC
             * 
             * srlevoe 11/3/2020 - ABOVE conversion was wrong
             * 
             * RIEEE - x86-linux vax intel (LSB)
             * IEEE - sun java PPC (MSB)
             * ***/
    		 if (pds3SampleType.equalsIgnoreCase("IEEE_REAL") && sample_bits == 32) {
    			 ...
    			 pds4data_type = "IEEE754MSBDouble" ; // 'SignedByte'
    			 ...
    			 pds4data_type = "IEEE754LSBDouble" ; // 'SignedByte'
    		 }
    	    ...
    	    **/
    		...
    		// these are from the RenderedImage which is JAVA and therefore always HIGH, big endian, MSB
    	 if (dataType == DataBuffer.TYPE_BYTE) pds4data_type = "UnsignedByte" ; // 'SignedByte'
         if (dataType == DataBuffer.TYPE_SHORT) pds4data_type = "SignedMSB2" ; // 'SignedLSB2'
         if (dataType == DataBuffer.TYPE_USHORT) pds4data_type = "UnsignedMSB2"; // 'UnsignedLSB2'
         if (dataType == DataBuffer.TYPE_INT) pds4data_type = "SignedMSB4" ; // 'UnsignedLSB4'  // no unsigned int??
         
         if (dataType == DataBuffer.TYPE_FLOAT) pds4data_type = "IEEE754MSBSingle";
         if (dataType == DataBuffer.TYPE_DOUBLE) pds4data_type = "IEEE754MSBDouble";
    	}
