...
public class VicarRenderedImage extends  SimpleRenderedImage {
	...
	public void setSourceYsubsample(int sourceYsubsample) {
	 ...
	 public void memoryInfoAndGC(String message, int gcCt) {
    	// Get the Java runtime
        Runtime runtime = Runtime.getRuntime();
        
        
    	for (int i=0 ; i< gcCt ;i++) {
        ...
        // Run the garbage collector
        if (debug) {
        	System.out.printf("memInfo %s gcCt=%d " , message, gcCt);	
	    	...
	    	System.out.printf(" memory free: %d M, " , (freeMemory / (1024 * 1024)));	
	    	System.out.printf("  allocated: %d M, ", (allocatedMemory / (1024 * 1024)));
	    	System.out.printf("  max: %d M, " ,(maxMemory / (1024 * 1024)) );
	    	System.out.printf("  total free: %d M \n", ((freeMemory + (maxMemory - allocatedMemory)) / (1024 * 1024)));
    	}
 ...
 /**
    ...
    */
    private Raster computeTile(int tileX, int tileY) {
        ...
        // turn off all subsampling code. Not in use
        boolean doSSprep = false;
        boolean doSS = false;
        
        int gcCt = 2;
        
        
        boolean debug_save = debug;
        // debug = true;
        ...
        if (ss_tWidth > ss_xleft) {
        	ss_tWidth = ss_xleft;
        	}
    	if (ss_tHeight > ss_yleft) {
    		ss_tHeight = ss_yleft;
    		}
        ...
        if (debug) {
        	...
        	System.out.println("XX VicarRenderedImage.computeTile doSSprep="+doSSprep+"   doSS="+doSS+"   "); 
        ...
        // memoryInfoAndGC("computeTile 1",gcCt) ;
        SampleModel ss_tileSampleModel = null;
        DataBuffer ss_dbuf = null;
        Rectangle ssTileRect =null;
        
        if (doSSprep) {
        	if (debug) {
            	System.out.println("doSS ss_tileSampleModel = vif.createSampleModel("+ss_tWidth+", "+ss_tHeight+"  ");
        	}
        	// sub sample versions of everything
	        ss_tileSampleModel = vif.createSampleModel(ss_tWidth, ss_tHeight);
	        theSSTile = Raster.createWritableRaster(ss_tileSampleModel, ss_org);          
	        ssTileRect = theSSTile.getBounds();              
	        ss_dbuf = theSSTile.getDataBuffer();
        }
        ...
        // memoryInfoAndGC("computeTile 2",gcCt) ;
        ...
        // memoryInfoAndGC("computeTile 3",gcCt) ;
        ...
        if (doSSprep ) {
	        ...
	        if (debug) {
	        	// debug test
	        	System.out.println("  doSSprep "+doSSprep+" ###############");
        ...
        // memoryInfoAndGC("computeTile doSS="+doSS+" doSSprep="+doSSprep+" ",gcCt) ;
