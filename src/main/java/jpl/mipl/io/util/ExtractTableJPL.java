...
public class ExtractTableJPL {
	...
	public void run(String[] args) {
		...
		if (debug) {
			System.out.println("run outputFile "+outputFile);
		}
		if (outputFile != null) {
			
			if (outputFile.getName().equalsIgnoreCase("string")) {
				if (debug) {
					System.out.println("run outputFile is string");
				}
			...
			// System.out.println("run writing to System.out");
		...
		for (Object obj : objectAccess.getTableObjects(fileArea)) {
			...
			if (listTables) {
				...
				// out may be a file or new PrintWriter(stringWriter);
	...
	private void displayRows(TableReader reader, FieldDescription[] fields, int[] displayFields, int[] fieldLengths) {
		...
		try {
			...
			System.err.println("Cannot parse the next table record: " + e.getMessage());
			System.exit(1);
		}
	}
	...
	private void parseArguments(String[] args) {
		...
		} // one was already set from somewhere
	...
	public String toString() {
		// System.out.printf("ExtractTableJPL.toString stringWriter = \n"+stringWriter+"\n");
