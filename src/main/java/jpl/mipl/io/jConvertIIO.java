...
// import jpl.mipl.jade.*;  // removed: jade-core replaced by DisplayJAI in JFrameJade
...
boolean writeTransparency = false; //the output image should have missing/invalid pixels as transparency
...
// used to hold PDS IMAGE VAID_MINIMUM AND VALID_MAXIMUM
// will be used on a rescale instead of getting histogram max min values
// if the default values (0.0, 0.0) are seen these values are ignored
...
// used in processFormat to override calculation
...
int tileSizeX = 256;
int tileSizeY = 256;
int fullWidthTileSize = -1;
// int tileSizeX = 0;
// int tileSizeY = 0;

static final double JAI_CACHE_DEFAULT = 0.75;
double jaiCache = -1;
...
/**
 * This specifies an extra file to be opened and read
 * How should this extra file be used
 * 1) The default case is this is an extra PDS/ODL formatted label
 * parse this label, add the XML to the ImageMetadata
 * Insert the metadata as another context object inn the PDS4 Generator call
 * srl - 3-2018
 */
String extraFilename = "";
String extraFileType = "ODL"; // "PVL", "csv", "xml", "json"
String extraFileUse = "velocity"; // "merge", "data"
String extraFileLabel= "extra"; // The variable name to be used by a velocity template to access these values
...
static private void setJAIMemory(double amount, boolean debug) {
    final long totalMemory = Runtime.getRuntime().maxMemory();
    long jaiCacheCapacity = 0;
    if (amount < 0) {
        jaiCacheCapacity = (long)(JAI_CACHE_DEFAULT * totalMemory);
    } else if (amount == 0) {
        jaiCacheCapacity = 0;
    } else if (amount <= 1) {
        jaiCacheCapacity = (long)(amount * totalMemory);
    } else {
        jaiCacheCapacity = (long)amount;
    }
	JAI.getDefaultInstance().getTileCache().setMemoryCapacity(jaiCacheCapacity);
    if (debug) System.out.printf("set JAI cache to %.2fMB\n", jaiCacheCapacity / (1024.0 * 1024.0));
}

static private int runParallelBatch(ExecutorService executor, List<Callable<Integer>> tasks)
    ...
    for (Future<Integer> f : futures) {
        int result = f.get();
        if (result >= 0) {
            return result; //return line number of first failing command
        }
    }
...
static public void main(String args[])
    ...
    setJAIMemory(-1, false); //default


	/****
	 * these settings could potentialy improve performance
	jai.getTileScheduler().setParallelism(7);
    jai.getTileScheduler().setPrefetchParallelism(7);
    jai.getTileScheduler().setPriority(5);
    jai.getTileScheduler().setPrefetchPriority(5);
	 ****/
	...
	final jConvertIIO convertOne = new jConvertIIO();
	...
	for (int i=0; i<args.length ;i++) {
        ...
        if (key.equalsIgnoreCase("PARALLEL_BATCH")) {
            if (value.equalsIgnoreCase("AVAILABLE_CORES")) {
                parallelBatchSize = Runtime.getRuntime().availableProcessors();
            } else {
                try {
                    parallelBatchSize = Integer.parseInt(value);
                } catch (NumberFormatException nfe) {
                    System.err.println("Error parsing PARALLEL_BATCH=" + value);
                }
            }
        }
	}
	...
	if (argFile.equalsIgnoreCase("") ) {
        ...
        List<Callable<Integer>> parallelBatch = null;
        Map<Integer, String> parallelBatchCommands = null;
        ExecutorService parallelExecutor = null;
        if (parallelBatchSize > 0) {
            parallelBatch = new ArrayList<Callable<Integer>>();
            parallelBatchCommands = new HashMap<Integer, String>();
            parallelExecutor = Executors.newFixedThreadPool(parallelBatchSize);
            System.out.println("parallel batch size: " + parallelBatchSize);
        }
		...
		while ((line = reader.readLine()) != null) {
			...
			if (lineTrimmed.length() == 0) {
                ...
                if (parallelBatchSize > 0) {
                    final int lineNum = argFileLine;
                    parallelBatch.add(new Callable<Integer>() {
                            @Override
                            public Integer call() throws Exception {
                                return (new jConvertIIO()).conv(theArgs) ? -1 : lineNum; //return line nuber on fail
                            }
                        });
                    parallelBatchCommands.put(argFileLine, lineTrimmed);
                    status = true;
                } else if (reuseConvert) {
					status = convertOne.conv(theArgs);
				} else {
                    status = (new jConvertIIO()).conv(theArgs);
				}

                if (parallelBatchSize > 0 && parallelBatch.size() == parallelBatchSize) {
                    int firstFail = runParallelBatch(parallelExecutor, parallelBatch);
                    if (firstFail >= 0) {
                        status = false;
                        argFileLine = firstFail;
                        lineTrimmed = parallelBatchCommands.get(firstFail);
                    }
                    parallelBatch.clear();
                    parallelBatchCommands.clear();
                }
				
				// print the input command line on error and exit
				if (status == false) {
            ...
            if (parallelBatchSize > 0 && parallelBatch.size() > 0) {
                int firstFail = runParallelBatch(parallelExecutor, parallelBatch);
                if (firstFail >= 0) {
                    System.out.println("line "+firstFail+") error: command Failed");
                    System.out.println(parallelBatchCommands.get(firstFail));
                    System.exit(1);
                }
                parallelBatch.clear();
                parallelBatchCommands.clear();
...
public void printHelp() {
    ...
    System.out.println("ARGFILE=F or ARG=f read command line arguments for a batch run one line at a time from given file");
    System.out.println("PARALLEL_BATCH=N run batches in parallel groups of up to N tasks at a time.  If N is the string AVAILABLE_CORES then detect and use the number of available cores.  Non-positive N disables parallelism.");
	...
	System.out.println("FULL_WIDTH_TILE_SIZE=N used in vicar, pds readers to set tile size equal to W,h where W is the input image width, H is the input image height, and h=min(H,max(1,floor(N/W)))");
	System.out.println("JAI_CACHE=N set JAI tile cache capacity.  N < 0 uses default fraction " + JAI_CACHE_DEFAULT + " of available heap memory.  0 disables JAI tile cache.  0 < N <= 1 uses the given fraction of available heap memory.  N > 1 uses the given number of bytes.");
...
//		  USE_PIRL
	    ...
	    else if (key.equalsIgnoreCase("WRITE_TRANSPARENCY")) {
	    	if (value == null || value.equalsIgnoreCase("true")) {
	    		writeTransparency = true;
	    	}
	    }
	    ...
	    else if (key.equalsIgnoreCase("RESCALE_CONSTANT") ) {
	    	// RESCALE_CONSTANT=constant  float value. default is 1.0, only used if OFORM is to a new data type
	    	if (debug) System.out.println("RESCALE_CONSTANT "+value);
	    	rescaleOnFormat = true;
	    	// double[] ff = getValuesDouble(value);
	    	// rescale_constant = ff[0];
	    	rescale_constant = Double.parseDouble(value);
	    	
	    	
	    	if (debug) System.out.println("RESCALE_CONSTANT "+rescale_constant);
	    	
	    }
	    else if (key.equalsIgnoreCase("RESCALE_OFFSET") ) {
	    	// RESCALE_OFFSET=constant  float value. default is 0.0, only used if OFORM is to a new data type
	    	if (debug) System.out.println("RESCALE_OFFSET "+value);
	    	rescaleOnFormat = true;
	    	// double[] ff = getValuesDouble(value);
	    	// rescale_offset = ff[0];
	    	rescale_offset = Double.parseDouble(value);
	    	
	    	if (debug) System.out.println("RESCALE_OFFSET "+rescale_offset);
	    }
	    else if (key.equalsIgnoreCase("VALID_MINIMUM") ) {
	    	// RESCALE_CONSTANT=constant  float value. default is 1.0, only used if OFORM is to a new data type
	    	if (debug) System.out.println("VALID_MINIMUM "+value);
	    	rescaleOnFormat = true;
	    	// double[] ff = getValuesDouble(value);
	    	// rescale_constant = ff[0];
	    	validMinimum = Double.parseDouble(value);
	    	
	    	if (debug) System.out.println("VALID_MINIMUM "+validMinimum);
	    } 
	    else if (key.equalsIgnoreCase("VALID_MAXIMUM") ) {
	    	// RESCALE_CONSTANT=constant  float value. default is 1.0, only used if OFORM is to a new data type
	    	if (debug) System.out.println("VALID_MAXIMUM "+value);
	    	rescaleOnFormat = true;
	    	// double[] ff = getValuesDouble(value);
	    	// rescale_constant = ff[0];
	    	validMaximum = Double.parseDouble(value);
	    	
	    	
	    	if (debug) System.out.println("VALID_MAXIMUM "+validMaximum);	
	    } 
        ...
        else if (key.equalsIgnoreCase("FULL_WIDTH_TILE_SIZE")) {
            try {					
                fullWidthTileSize = Integer.parseInt(value);
            }
            catch (NumberFormatException nfe) {
                System.out.println("FULL_WIDTH_TILE_SIZE NumberFormatException "+nfe);
            }
	    	if (debug) System.out.println("FULL_WIDTH_TILE_SIZE"+value);
        }
        else if (key.equalsIgnoreCase("JAI_CACHE")) {
            try {					
                jaiCache = Double.parseDouble(value);
            }
            catch (NumberFormatException nfe) {
                System.out.println("JAI_CACHE NumberFormatException "+nfe);
            }
	    	if (debug) System.out.println("JAI_CACHE"+value);
        }
	 ...
	 // velocity engine configuration file directory. Make a vicar ENV variable for a select??
	 ...
	 /* extra file that can be used in many ways
	  * 1) use to add extra label information to a velocity template
	  * 2) merge values into an ooutput label, PDS/ODL3 or vicar
	  * 3) another data file. Maybe csv
	  */
	    else if (key.equalsIgnoreCase("EXTRA_FILE_NAME") ) {
	       if (value != null && !value.isEmpty()) 
	    	{
	    		extraFilename = value; // null indicates no value
	    	}
	    }
	    // indictes how the above EXTRA_FILE_NAME will be used, parsed
	    else if (key.equalsIgnoreCase("EXTRA_FILE_TYPE") ) {
		       if (value != null && !value.isEmpty()) 
		    	{
		    		extraFileType = value.toUpperCase(); // null indicates no value
		    	}
		    }
	    else if (key.equalsIgnoreCase("EXTRA_FILE_USE") ) {
		       if (value != null && !value.isEmpty()) 
		    	{
		    		extraFileUse = value.toUpperCase(); // null indicates no value
		    	}
		    }
	    else if (key.equalsIgnoreCase("EXTRA_LABEL") ) {
		       if (value != null && !value.isEmpty()) 
		    	{
		    		extraFileLabel = value.toLowerCase(); // null indicates no value
		    	}
		    }
	...
	**/ 
	 ...
	 try {
		...
		if (debug) System.out.println("jConvertIIO "+ outputFormat+" ************************");	
		
		// for some output formats fullRead will tolerate read exceptions and continue anyway
	    ...
	    if (readerFormat.equalsIgnoreCase("pds") ) {
			fakeImage = ((PDSImageReader) reader).getFakeImageNoRead();
			/* the command line may have supplied a value which should overide what is in the input file label */
			if (validMaximum == 0.0) {
				validMinimum = ((PDSImageReader) reader).getValidMinimum();
				validMaximum = ((PDSImageReader) reader).getValidMaximum();
			}
			if (debug) {
        ...
        if (fullWidthTileSize > 0) {
            tileSizeX = rii.getWidth();
            tileSizeY = Math.min(rii.getHeight(), Math.max(1, fullWidthTileSize / tileSizeX));
            if (debug) System.out.println("setting full width tile size " + tileSizeX + "x" + tileSizeY);
            imUtil.setTileSizeX(tileSizeX);
            imUtil.setTileSizeY(tileSizeY);
            if (irp instanceof VicarImageReadParam) {
                ((VicarImageReadParam) irp).setTileSizeX(tileSizeX);
	    ...
	    if (readerFormat.equalsIgnoreCase("pds4table")) {
			...
			if (debug) { 
				System.out.println("jConvertIIO line 2406  outputFileName = "+outputFileName ); 
				}	
	 ...
	 if (debug) { 
		 ...
		 System.out.println(" extraFilename = "+extraFilename);
	 	 System.out.println(" extraFileType = "+extraFileType);
	 	 System.out.println(" extraFileUse = "+extraFileUse);
	 	 System.out.println(" extraFileLabel = "+extraFileLabel);
		 System.out.println("#################################");
	 }
	 if (!extraFilename.equals("") && 
			 (extraFileType.equalsIgnoreCase("ODL") || extraFileType.equalsIgnoreCase("PVL") || extraFileType.startsWith("PDS") )) {
		 // parse this file and add it to the metedata object
		 if (debug) { 
			 System.out.printf("read the extra file extraFilename=%s \n", extraFilename);	
		 }
		 IIOImage extraIioImage = null;
		 try {
			 ImageUtils extraImUtil = new ImageUtils(extraFilename) ;
			 // extraImUtil.setDebug(true);
			 extraImUtil.setDebug(debug);
			 extraIioImage = extraImUtil.fullRead(extraFilename, null);
		} catch (IOException e) {
			// TODO Auto-generated catch block
		...
		if (debug) { 
			...
			System.out.printf(" extraIioImage "+extraIioImage+"  im "+im);
		}
		
		if (extraIioImage != null) {
			// this is the native data from thr reader
			IIOMetadata extraIm = extraIioImage.getMetadata();
			String nativeFormatName = extraIm.getNativeMetadataFormatName();
			String extraFormatName = "EXTRA_LABEL";
			if (debug) { 
				System.out.printf(" nativeFormatName=%s extraFilename=%s extraFormatName=%s\n", nativeFormatName, extraFilename, extraFormatName);
			}
			Node extraNode = null;
			try {
				extraNode = extraIm.getAsTree(nativeFormatName);
			
				// add the metadata from this extra label
				im = iioImage.getMetadata();
				if (debug) { 
					System.out.printf(" extraIioImage "+extraIioImage+"  im "+im);
				}
			...
			if (debug) { 
				...
				// im.setFromTree(formatName, root);
		 		System.out.println("readerFormat = "+readerFormat);
		 		System.out.println("im = "+im);
			}
			...
			DOMutils domUtils = new DOMutils();
	 		...
	 		if (debug) { 			
		 		...
		 		for (int i=0 ; i< formats.length ;i++) {
		 			String formatName = formats[i];
		 			// docName = docName + formatName+"_";
		 			docName = String.format("%s.xml", formatName);
		 			System.out.println(" formats["+i+"] " +formats[i]+" - "+formatName+" docName = "+docName); 		
		 		...
		 		extraFormats = im.getExtraMetadataFormatNames();
		 		...
		 		if (extraFormats == null) {
			 		...
			 		for (int i=0 ; i< extraFormats.length ;i++) {
			 			...
			 			docName = String.format("%s.xml", formatName);
			 			System.out.println(" extraFormats["+i+"] " +formats[i]+" - "+formatName+" docName = "+docName); 	
			 		}
		 		}
	 		}
	 	} 
	 ...
	 if (pdsDetachedOnly == true && 
	    ...
	    if (debug) { 
	    	System.out.println("********************************************"); 
	    	System.out.println("PDS detached label write calling processSave"); 
	    	}
    ...
    if (debug) 
	  ...
	  if (printInfo || debug) {
		...
		System.out.println(" rescale_constant "+rescale_constant+"   rescale_offset "+rescale_offset+" ");
		System.out.println(" validMinimum "+validMinimum+"  validMaximum "+validMaximum+"  ");
	 ...
	 imageHasBeenProcessed = false;
	  ...
	  if (outputDataTypeString.equals("DEFAULT")) {
	    ...
	    if (validMaximum != 0.0 || (rescale_constant != 1.0 && rescale_offset != 1.0) ) {
	    	if (debug) {
	    	  	System.out.println("calling processFormat when outputDataTypeString.equals('DEFAULT') ");
	    	}
	    	processedImage = processFormat(filteredImage, outputDataType, rescaleOnFormat, rescale_constant, rescale_offset);
	    ...
	    // processFormat(RenderedImage image, int newDataType, boolean rescaleOnFormat, double rescale_constant, double rescale_offset) {
	    processedImage = processFormat(filteredImage, outputDataType, rescaleOnFormat, rescale_constant, rescale_offset);
	    imageHasBeenProcessed = true;	    
	    }
	   ...
	   if (debug) {
		...
		// if (outputFormat != null && outputFormat.equalsIgnoreCase("pds")) {
		 if (outputFormat != null && outputFormat.toLowerCase().startsWith("pds")) {
			...
			System.out.println(" writeTransparency "+writeTransparency);
		}
	   }
	...
	if (outputFormat == null  && (outputFileName.equalsIgnoreCase("-") == false) &&
	   ...
	   String dts = dataTypeString(processedImage) ;
		...
		if (cm1 == null && 
				...
				(((numBands == 3 || numBands == 1) && !writeTransparency) || 
				 ((numBands == 4 || numBands == 2) && writeTransparency))) {
				if (debug) {System.out.println("ColorModel is null "+cm1+" numBands = "+numBands+" outputFormat "+outputFormat);}
				...
				if (sm1 instanceof ComponentSampleModelJAI || sm1 instanceof ComponentSampleModel) {
					...
					if (cm1 == null) {
						...
						if (cm1 == null) {
							...
							{
								if (debug) {System.out.println("Attempting to write transparency");} 
								transparency = Transparency.TRANSLUCENT;
								hasAlpha = true;
							}

							if (numBands == 3 || numBands == 4) {
								if (debug) {System.out.println("ColorModel using rgb settings");}   
								java.awt.color.ColorSpace colorSpace = ICC_ColorSpace.getInstance(ICC_ColorSpace.CS_sRGB);		              
									cm1 = new ComponentColorModel(colorSpace, hasAlpha, isAlphaPremultiplied, transparency, sm1.getTransferType());
								} else  if (numBands == 1 || numBands == 2) {
								java.awt.color.ColorSpace colorSpace = ICC_ColorSpace.getInstance(ICC_ColorSpace.CS_GRAY);
								if (debug) {System.out.println("ColorModel using grayscale settings");}              
								cm1 = new ComponentColorModel(colorSpace, hasAlpha, isAlphaPremultiplied, transparency, sm1.getTransferType());
							} 
						...
						if (debug) {System.out.println("Creating colormodel just using sample model");}
						cm1 = ImageCodec.createComponentColorModel(sm1);
		          }
...
/**
    ...
    ****************/
    public void processSave(IIOImage iioImage, String inputFile, String outputFile, String format) {   
        ...
        if (debug) {
        	System.out.println( "jConvertIIO.processSave() inputFile="+inputFile+" outputFile="+outputFile+" output format is: "+format);
        }
        writer = null;
        DOMutils domUtils = new DOMutils();
        ...
        String outputFileName = "";
        String inputFileName = "";
        String inputFileNameAbsolutePath = "";
    	...
    	inputFileNameAbsolutePath = fin.getAbsolutePath();
    	// get the full path??
        ...
        if (debug) {
        	...
        	System.out.println( "jConvertIIO.processSave() inputfile: "+inputFile +"  "+inputFileNameAbsolutePath+ "  ");
        }
        ...
        if (format.equalsIgnoreCase("vicar") == true || format.equalsIgnoreCase("vic") == true ) {
            ...
            *******/	
			...
			((PDSImageWriteParam)  writeParam).setInputFileFullPath(inputFileNameAbsolutePath);
			...
			// add something to allow, force or disable native
                 ...
                 format.equalsIgnoreCase("btf") || format.equalsIgnoreCase("btiff")) {
					// put tif specific things into the ImageWriteParam
			
            if ((format.equalsIgnoreCase("btf") || format.equalsIgnoreCase("btiff")) && 
                writeParam instanceof it.geosolutions.imageio.plugins.tiff.TIFFImageWriteParam) {
                ((it.geosolutions.imageio.plugins.tiff.TIFFImageWriteParam)writeParam).setForceToBigTIFF(true);
            }
      	...
      	// else if (format.equalsIgnoreCase("pds") == true  ) {
        	...
        	if (debug) {
        		...
        		System.out.println( "inputFileName "+inputFileName);
        		System.out.println( "inputFileNameAbsolutePath "+inputFileNameAbsolutePath);
        		System.out.println( "readerFormat = "+readerFormat);
        		System.out.println( "useOutputFilename "+useOutputFilename);
        		System.out.println( "outputFileName "+outputFileName);
        		System.out.println( "outputFormat = "+outputFormat);
        	...
        	if (writeParam instanceof PDSImageWriteParam) { 
            	...
            	if (inData instanceof VicarMetadata) {
					...
					// only set this if it is "JSON" ??
					((PDSImageWriteParam)  writeParam).setExtraFileType(extraFileType);
					((PDSImageWriteParam)  writeParam).setExtraFilename(extraFilename);
...
//				 PDS detached label support
				...
				***/
				...
				// if (format.equalsIgnoreCase("pds4"))
				((PDSImageWriteParam)writeParam).setInputFileName( inputFileName);
				((PDSImageWriteParam) writeParam).setInputFileFullPath(inputFileNameAbsolutePath);
         ...
         if (transcoders.hasNext()) {
         	...
         	***/
         	...
         	if (debug) {
         		...
         		// serialize the inData 
         		String nativeFormatName = inData.getNativeMetadataFormatName();
        		System.out.println("nativeFormatName "+nativeFormatName+" ");
        		System.out.println("inData "+inData+" ");
                Document doc =  (Document) inData.getAsTree(nativeFormatName);
         		
         		domUtils.serializeDocument(doc, "preTranscodedMetadata.xml", "xml");
         	}
         	ImageTypeSpecifier imageType = new ImageTypeSpecifier(ri); // better ???
         	 ...
         	 if (transcodedMetadata != null) {
				...
				if (debug) {
            		...
            		String nativeFormatName = transcodedMetadata.getNativeMetadataFormatName();
            		System.out.println("transcodedMetadata "+transcodedMetadata);
            		System.out.println("nativeFormatName "+nativeFormatName+" ");
                    Document doc =  (Document) transcodedMetadata.getAsTree(nativeFormatName);
                    System.out.println("nativeFormatName "+nativeFormatName+",  document "+doc);
                    final Node root = doc.getDocumentElement();
                    System.out.println("root "+root);
					
					domUtils.serializeDocument(doc, "transcodedMetadataDoc.xml", "xml");					
					Node doc2 =  (Document) transcodedMetadata.getAsTree(nativeFormatName);              	
                	domUtils.serializeNode(doc2, "transcodedMetadataNode.xml", "xml");
				}
            ...
            * for now we'll use the code below which differs for pds or vicar formats
            ...
            if (fout != null && fout.equals("-")) {
            	...
            	if (isWebdav == true) {
                    ...
                    if (!(ios instanceof FileChannelImageOutputStream)) {
                        ios.close();
                        FileChannel fc =
                            FileChannel.open(fout_x.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE, 
                                             StandardOpenOption.READ, StandardOpenOption.TRUNCATE_EXISTING);
                        ios = new FileChannelImageOutputStream(fc);
                        if (debug) {
                            System.out.println("replaced ios with " + ios.getClass().getName());
                        }
                    }
            	}
            ...
            if (debug) {
             ...
             if (format.equalsIgnoreCase("vicar") == true || format.equalsIgnoreCase("pds") == true ||
				...
				if (debug) {
                    ...
                    System.out.println((new Date()) + "beginning write using " + writer.getClass().getName());
				}
            	writer.write(null, iioImage, writeParam); // PDS NEEDS WriteParam
 ...
 // convert the input image to a different DataType
    public RenderedImage processFormat(RenderedImage image, int newDataType, boolean rescaleOnFormat) {
    	RenderedImage formatedImage = processFormat( image, newDataType, rescaleOnFormat, 1.0, 0.0);
    	return (formatedImage);    
    }
    	
    	
    	
    public RenderedImage processFormat(RenderedImage image, int newDataType, boolean rescaleOnFormat, double rescale_constant, double rescale_offset) {
      ...
      * The default is TRUE
      * If the user has supplied either validMinimum, validMaximum or rescale_constant, rescale_offset
      * then the dats should be rescaled even if the data type doesn't change.
      * 
      */   
    // don't check if we should do this or not. Even of the data type is the same a rescale may help the data look better
    /**
    if (rescaleOnFormat == true && validMinimum == 0.0 && validMaximum == 0.0 && rescale_constant == 1.0 && rescale_offset == 0.0) {
	...
	***/
    if (rescaleOnFormat) {
    	...
    	ParameterBlock PB=new ParameterBlock();
    	if (validMinimum != 0.0 || validMaximum != 0.0) {
    		double ceiling = getMaxForDataType(newDataType) ;
    		constant[0] = ceiling /(validMaximum-validMinimum);
			offset[0] = validMinimum * constant[0] * -1.0; // offset is added only for unsigned ??
			// 
			...
			if (debug) {
				System.out.println("processFormat(0.2) constant[0]="+constant[0]+"  offset[0]="+offset[0]+ " ceiling = "+ceiling);		
				double min1 = (validMinimum * constant[0]) + offset[0];
				double max1 = (validMaximum * constant[0]) + offset[0];
				System.out.println("processFormat(0.3)  validMinimum="+validMinimum+"  validMaximum="+validMaximum+" ");
			}
    	}
    	else if (rescale_constant == 1.0 && rescale_offset == 0.0) {
			...
			double ceiling = getMaxForDataType(newDataType) ;
		    double floor = getMinForDataType(newDataType) ;
		
        
			// double ceiling=Short.MAX_VALUE*2;
			// double max=0.0,min=ceiling;
			double max=floor,min=ceiling;
			for(int i=0;i<scale[0].length;i++){
		    	max=Math.max(max,scale[1][i]);
		    	min=Math.min(min,scale[0][i]);
			}
			if (debug) {
				System.out.println("processFormat(1) extrema scale   min="+scale[0][0]+"  max="+scale[1][0]+" floor="+floor+" ");
			...
			if (max == 0.0 && min == 0.0) { // ???
				...
				min = floor;
				if (debug) {
					System.out.println("processFormat(1) new min "+min+"  max "+max);
				}
			}
			/*******/
    		...
    		// use the user provided values RESCALE_CONSTANT=constant RESCALE_OFFSET=offset 
    		// is this correct for a multiband image??
    		constant[0] = rescale_constant;
    		offset[0] = rescale_offset;
    		if (debug) {
				System.out.println("processFormat(2a) constant="+constant[0]+"  offset="+offset[0]);		
			}
    	}
