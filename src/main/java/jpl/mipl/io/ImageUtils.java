...
import jpl.mipl.io.streams.FileChannelImageInputStream;
...
public BufferedImage fakeBufferedImage() {
	
	RenderedImage ri = null;
	int width = 1;
	int height = 1;
	BufferedImage bi = new BufferedImage(width, height, java.awt.image.BufferedImage.TYPE_INT_RGB);
	return bi;
}
       ...
       try {
        ...
        if (debug) System.out.println("readerFormat "+readerFormat+"   readerClassName "+readerClassName+"  debug = "+debug);
 		if (reader instanceof PDSImageReader ) {	
 			...
 			***/
 		} else if (readerFormat.contentEquals("pds")) {	
		...
		***/
		...
		if (getAsRenderedImage) {
			...
			try {
				...
				** pds4table **/
				...
				if (debug) {
					System.out.println("reader.readAsRenderedImage(imageIndex, param) "+ outputFormat+" ************************");	
				}
				try {
					renderedImage = reader.readAsRenderedImage(imageIndex, param);	
				} catch (Exception e) {
					// TODO Auto-generated catch block
					System.out.println("reader.readAsRenderedImage(imageIndex, param)  "+e.getMessage());	
					e.printStackTrace();
					if (outputFormat.equalsIgnoreCase("PDS4")) {
						fakeImage = true;
						// continue make a fake BufferedImage
					}
				}
				...
				if (fakeImage == true) {
					...
					bufferedImage = fakeBufferedImage();
					System.out.println("format = "+outputFormat+" created BufferedImage to continue");
				}
				else if (renderedImage == null ) {
					if (debug) {
		...
		// this forces the reader to read in and store the metadata
         ...
         if (bufferedImage != null) {
        	 ...
        	 System.out.println("new IIOImage(renderedImage, null, im) renderedImage = "+renderedImage+"  ******************");	
        	 try {
        		 iioImage  = new IIOImage(renderedImage, null, im);
        	 } catch (Exception e) {
        		 System.out.println("new IIOImage(renderedImage, null, im)  "+e.getMessage());	
				 e.printStackTrace();
	...
	ImageInputStream iis = null;
	 ...
	 try {
	         ...
	         if (iis != null) { 
                 ...
                 if (!(iis instanceof FileChannelImageInputStream)) {
                     iis.close();
                     iis = new FileChannelImageInputStream(FileChannel.open(f.toPath()));
                     if (debug) {
                         System.out.println("replaced iis with " + iis.getClass().getName());
                     }
                 }
