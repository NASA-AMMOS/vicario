...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="VICAR_LABEL">
		<PDS_LABEL>
			...
			<!-- addded for MSAM support of mastcam -->
		...
		<xsl:if test="@name!='PROPERTY'">
			...
			<xsl:if test="count(subitem)=0">
				<xsl:choose>
					...
					<xsl:when
						test="parent::PROPERTY[@name ='IMAGE_DATA'] and contains(@name,'LINE_SAMPLES')">
						<HIDE>remove IMAGE_DATA LINE_SAMPLES</HIDE>
					</xsl:when>
					
					<xsl:when
						test="parent::PROPERTY[@name ='IMAGE_DATA'] and contains(@name,'BANDS')">
						<HIDE>remove IMAGE_DATA BANDS</HIDE>
					</xsl:when>
