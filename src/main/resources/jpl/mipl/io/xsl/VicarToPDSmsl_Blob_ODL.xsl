				...
				<xsl:choose>
					...
					<xsl:when test="not(contains(@name, '__UNIT'))">
						<xsl:element name="item">
							...
							<!-- 
							While in the midst of the Covid-19 pandemic, we've decided to ensure
							that nodes with unknown values (UNK, N/A, NULL) do not keep their
							units attribute if units were indeed included.  08/03/20 - nttoole
						    -->
							<xsl:choose>
							...
							<xsl:choose>
								...
								<xsl:when test="starts-with(.,'nan')">
									<xsl:attribute name="quoted">false</xsl:attribute>
									<xsl:value-of select="." />
								</xsl:when>
				
								<xsl:when test="contains(.,'nan.0')">
									<xsl:attribute name="quoted">false</xsl:attribute>
									<xsl:value-of select="." />
								</xsl:when>					
