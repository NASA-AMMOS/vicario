...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="VICAR_LABEL">
		<PDS_LABEL>
			...
			<!-- Added March 2019 for M20 G6.0 Delivery -->
			<xsl:apply-templates select="PROPERTY[@name='TURRET_COORDINATE_SYSTEM']" />                       
			<xsl:apply-templates select="PROPERTY[@name='TOOL_COORDINATE_SYSTEM']" />
			...
			<!-- Adding missing groups that are appearing in M20 labels, December 2020 -->
			<xsl:apply-templates select="PROPERTY[@name='LED_ILLUMINATION_PARMS']" />
			<xsl:apply-templates select="PROPERTY[@name='SLI_ILLUMINATION_PARMS']" />
			<xsl:apply-templates select="PROPERTY[@name='PIXL_SENSOR_COORDINATE_SYSTEM']" />
			<xsl:apply-templates select="PROPERTY[@name='HELI_G_COORDINATE_SYSTEM']" />
			<xsl:apply-templates select="PROPERTY[@name='HELI_M_COORDINATE_SYSTEM']" />
			<xsl:apply-templates select="PROPERTY[@name='HELI_S1_COORDINATE_SYSTEM']" />
			<xsl:apply-templates select="PROPERTY[@name='HELI_S2_COORDINATE_SYSTEM']" />
			<xsl:apply-templates select="PROPERTY[@name='SCAM_MINI_HEADER']" />
			<xsl:apply-templates select="PROPERTY[@name='DRILL_ARTICULATION_STATE']" />
			<xsl:apply-templates select="PROPERTY[@name='SCS_ARTICULATION_STATE']" />
			<xsl:apply-templates select="PROPERTY[@name='SHA_ARTICULATION_STATE']" />
			<xsl:apply-templates select="PROPERTY[@name='ACI_SCANNER_ARTICULATION_STATE']" />
			<xsl:apply-templates select="PROPERTY[@name='ZSTACK_REQUEST_PARMS']" />
			<xsl:apply-templates select="PROPERTY[@name='PIXL_BASE_COORDINATE_SYSTEM']" />
			<xsl:apply-templates select="PROPERTY[@name='VIDEO_REQUEST_PARMS']" />
			<xsl:apply-templates select="PROPERTY[@name='VIDEO_PARMS']" />
			<xsl:apply-templates select="PROPERTY[@name='ROW_SUM_REQUEST_PARMS']" />
			<xsl:apply-templates select="PROPERTY[@name='LL_COORDINATE_SYSTEM']" />
	...
	<xsl:template match="PROPERTY[@name='TURRET_COORDINATE_SYSTEM']">
		<comment>/* COORDINATE SYSTEM STATE: TURRET */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='TOOL_COORDINATE_SYSTEM']">
		<comment>/* COORDINATE SYSTEM STATE: TOOL */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	...
	<xsl:template match="PROPERTY[@name='LED_ILLUMINATION_PARMS']">
		<comment>/* LED ILLUMINATION PARAMETERS */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='SLI_ILLUMINATION_PARMS']">
		<comment>/* SLI ILLUMINATION PARAMETERS */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='PIXL_SENSOR_COORDINATE_SYSTEM']">
		<comment>/* PIXL SENSOR COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='HELI_G_COORDINATE_SYSTEM']">
		<comment>/* HELI G COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='HELI_M_COORDINATE_SYSTEM']">
		<comment>/* HELI M COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='HELI_S1_COORDINATE_SYSTEM']">
		<comment>/* HELI S1 COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='HELI_S2_COORDINATE_SYSTEM']">
		<comment>/* HELI S2 COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>	
	<xsl:template match="PROPERTY[@name='SCAM_MINI_HEADER']">
		<comment>/* SCAM MINI HEADER */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='DRILL_ARTICULATION_STATE']">
		<comment>/* DRILL ARTICULATION STATE */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='SCS_ARTICULATION_STATE']">
		<comment>/* SCS ARTICULATION STATE */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='SHA_ARTICULATION_STATE']">
		<comment>/* SHA ARTICULATION STATE */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='ACI_SCANNER_ARTICULATION_STATE']">
		<comment>/* ACI SCANNER ARTICULATION STATE */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='ZSTACK_REQUEST_PARMS']">
		<comment>/* ZSTACK REQUEST PARAMETERS */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='PIXL_BASE_COORDINATE_SYSTEM']">
		<comment>/* PIXL BASE COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='VIDEO_REQUEST_PARMS']">
		<comment>/* VIDEO REQUEST PARAMETERS */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='VIDEO_PARMS']">
		<comment>/* VIDEO PARAMETERS */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
	</xsl:template>
	<xsl:template match="PROPERTY[@name='LL_COORDINATE_SYSTEM']">
		<comment>/* LL COORDINATE SYSTEM */</comment>
		<GROUP>
			<xsl:attribute name="name"><xsl:value-of select="@name" /></xsl:attribute>
			<xsl:apply-templates />
		</GROUP>
				...
				<xsl:choose>
					...
					<xsl:when test="not(contains(@name, '__UNIT'))">
						<xsl:element name="item">
							...
							<!-- 
							For nodes with unknown values (UNK, N/A, NULL), do not attempt to retain units. 08/03/20 - nttoole
							-->
							<xsl:choose>
								<xsl:when test="not( . = 'UNK' or . = 'N/A' or . = 'NULL' )">
							...
							<xsl:choose>
								...
								<!-- Test for empty-string value first, and force as a quoted empty-string -->
								<xsl:when test="string-length(.)=0">
									<xsl:attribute name="quoted">true</xsl:attribute>
									<xsl:value-of select="." />
								</xsl:when>
		
								<!-- Special handling for mixed-quoted strings 
									 Context: Vicar tools themselves get triggered when they see a value starting with E, 
											  believing that the value must be a number.  So MIPL's work-around is to wrap 
											  values that start with E with an extra set of double-quotes (within the 
											  existing single-quotes that are standard for Vicar string values).
									Example: SOURCE_PRODUCT_ID='"EAE_0116T0677236090_353ECM_N0110000EDLC00000_0000LUJ01"'
									DevTODO: Double-quotes seem bad in general, consider a basic check for it?  
								-->
								<xsl:when test="starts-with(.,$doubleQuote)">
									<xsl:attribute name="quoted">true</xsl:attribute>
									<xsl:value-of select="translate(.,$doubleQuote,'')"/>
								</xsl:when>

								<!-- Placeholder in case we see the opposite in the future:  "'E_STRING'"
								<xsl:when test="starts-with(.,$singleQuote)">
										<xsl:variable name="sin-quoted-length" select="string-length(.)"/>
										<xsl:attribute name="quoted">true</xsl:attribute>
										<xsl:value-of select="substring(.,2,($sin-quoted-length - 2))"/>
								</xsl:when>
								-->
                                ...
                                <xsl:when test="@name='SUPPLEMENTAL_MASK_FILE'">
                                    <xsl:attribute name="quoted">true</xsl:attribute>
                                    <xsl:value-of select="." />
                                </xsl:when>

                                <xsl:when test="@name='RMI_CMOS_REGISTERS'">
                                    <xsl:attribute name="quoted">true</xsl:attribute>
                                    <xsl:value-of select="." />
                                </xsl:when>
